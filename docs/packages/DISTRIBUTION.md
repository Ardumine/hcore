...
HCore is a microkernel with dynamically loaded packages. The kernel repo (`hcore/`) contains **only the kernel + the ABI — no filesystem** (the `hcore` binary is portable, like `vmlinuz`; it ships no rootfs). The layout is **flat** (no `src/`):

```
hcore/
  HCore.Main/               ← kernel
  HCore.Modules.Base/       ← the ABI (git submodule → ardumine/hcore-modules-base)
  scripts/build-base-fs.sh  ← assembles the base FS release artifact
```

`HCore.Modules.Robotics` (domain contracts like `ILidar`) is **its own repo**
(`ardumine/hrobotics`) and is no longer part of the kernel; cross-module identity for it
is preserved by the kernel's shared-contract loader (any `HCore.Modules.*` assembly a
package loads is promoted into the `Default` ALC).
...
ardumine/
  hcore/                ← kernel
  hcore-modules-base/   ← HCore.Modules.Base (the ABI; submodule of hcore + every package)
  hrobotics/            ← HCore.Modules.Robotics (shared domain contracts; bundles Base)
  hinit/         ← HCore.Packages.HInit (PID 1)
  hshell/        ← HCore.Packages.HShell (interactive shell)
  hpm/           ← HCore.Packages.Hpm (package manager)
  hsensors/      ← HCore.Packages.Sensor (LIDAR + SLAM; uses hrobotics)
...
Each package consumes the `HCore.Modules.Base` ABI as a git submodule (pinned to a tag) and deploys its build output to `hcore/FS/packs/` via PostBuild.
...
The list of essential packages lives in `HCore.Main/Bootstrap/bootstrap.json`. It is the **single source of truth** for what a base system needs — consumed by both the in-kernel network self-heal *and* the base-FS assembler script:
...
3. **Run the kernel:**
   ...
   # Or override FS root:
   dotnet run --project HCore.Main -- --fs=/path/to/custom/FS
   ```
...
dotnet publish HCore.Main -c Release -r linux-x64 \
  --self-contained true -p:PublishSingleFile=true -o publish/

# Framework-dependent (77KB, needs .NET 10 runtime)
dotnet publish HCore.Main -c Release -o publish/
```
...
```bash
dotnet run --project HCore.Main -- --fs=/opt/hcore/fs
# or
HCORE_FS_ROOT=/opt/hcore/fs dotnet run --project HCore.Main
```
